<script lang="ts">
	...
	function setupTelegramSDK() {
		...
		try {
			...
			const tg = (
				window as unknown as { Telegram?: { WebApp?: { disableVerticalSwipes?: () => void } } }
			).Telegram;
...
<div class="min-h-screen bg-background font-sans text-foreground antialiased">
	{#if isLoading}
		<div class="flex min-h-screen flex-col items-center justify-center p-6 text-center">
			<Loader2 class="mb-4 size-8 animate-spin text-muted-foreground" />
		...
		<div class="flex min-h-screen items-center justify-center p-6">
			<Card.Root class="w-full max-w-md">
				<Card.Header class="flex flex-col items-center gap-4 text-center">
					<div
						class="flex size-12 items-center justify-center rounded-xl border bg-card text-foreground"
					>
						<CarMinderLogo class="size-7" />
					</div>
					<div class="flex flex-col gap-1.5">
						<Card.Title class="text-xl font-semibold tracking-tight">car minder</Card.Title>
						<Card.Description class="text-sm leading-relaxed text-balance text-muted-foreground">
				...
				<Card.Content class="flex flex-col gap-4 pt-2">
					{#if isBotDisabled}
						<div
							class="flex items-center gap-2.5 rounded-lg border border-warning/30 bg-warning/10 p-3.5 text-xs leading-relaxed text-warning"
						>
					...
					{:else}
						<div
							class="rounded-lg border bg-muted/50 p-3 text-center text-xs text-muted-foreground"
						>
				...
				<Card.Footer
					class="justify-center border-t border-border/50 pt-4 text-xs text-muted-foreground"
				>
	...
	{:else if authError}
		<div class="flex min-h-screen items-center justify-center p-6">
			<Card.Root class="w-full max-w-md">
				<Card.Header class="flex flex-col items-center gap-4 text-center">
					<div
						class="flex size-12 items-center justify-center rounded-xl border bg-card text-destructive"
					>
						<ShieldAlert class="size-6" />
					</div>
					<div class="flex flex-col gap-1.5">
						<Card.Title class="text-xl font-semibold tracking-tight"
							>{m.tma_auth_error_title()}</Card.Title
						>
						<Card.Description class="text-sm leading-relaxed text-balance text-muted-foreground">
							{authError}
						</Card.Description>
